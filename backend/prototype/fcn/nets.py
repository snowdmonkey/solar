"""shamelessly copy the code from
...
# This function is just doing a center crop with (l-m)/2 left at both ends
...
class TransitionUp(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convTrans = nn.ConvTranspose2d(in_channels=in_channels,
                                            ...
                                            padding=0,
                                            bias=True)
        # crop = 'valid' means padding=0. Padding has reverse effect for transpose conv (reduces output size)
...
def fc_dense_net57(n_classes: int, channels: int) -> FCDenseNet:
    return FCDenseNet(in_channels=channels, down_blocks=(4, 4, 4, 4, 4),
                      up_blocks=(4, 4, 4, 4, 4), bottleneck_layers=4,
                      growth_rate=12, out_chans_first_conv=48, n_classes=n_classes)


def fc_dense_net67(n_classes: int, channels: int) -> FCDenseNet:
    return FCDenseNet(in_channels=channels, down_blocks=(5, 5, 5, 5, 5),
                      up_blocks=(5, 5, 5, 5, 5), bottleneck_layers=5,
                      growth_rate=16, out_chans_first_conv=48, n_classes=n_classes)


def fc_dense_net103(n_classes, channels: int):
